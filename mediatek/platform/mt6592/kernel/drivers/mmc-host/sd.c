...
u32 msdc_dump_padctl0(u32 id) //clk
{	
	u32 reg = 0;
	u32 tmp = 0;
	switch(id){
		case 0:
			...
			reg |= tmp << 8;
			sdr_get_field(MSDC0_GPIO_CLK_BASE, GPIO_MSDC_DRVN, tmp);
			reg |= tmp << 0;
			break;
		case 1:
			...
			reg |= tmp << 17;
			sdr_get_field(MSDC1_GPIO_CLK_BASE, GPIO_PUPD_MASK | GPIO_R1_MASK | GPIO_R0_MASK, tmp);
			reg |= tmp << 8;
			sdr_get_field(MSDC1_GPIO_CLK_BASE, GPIO_MSDC_DRVN, tmp);
			reg |= tmp << 0;
			break;
		case 2:
			...
			reg |= tmp << 17;
			sdr_get_field(MSDC2_GPIO_CLK_BASE, GPIO_PUPD_MASK | GPIO_R1_MASK | GPIO_R0_MASK, tmp);
			reg |= tmp << 8;
			sdr_get_field(MSDC2_GPIO_CLK_BASE, GPIO_MSDC_DRVN, tmp);
			reg |= tmp << 0;
			break;
		default:
			break;
...
{	
	...
	switch(id){
		case 0:
			...
			reg |= tmp << 8;
			sdr_get_field(MSDC0_GPIO_CMD_BASE, GPIO_MSDC_DRVN, tmp);
			reg |= tmp << 0;
			break;
		case 1:
			sdr_get_field(MSDC1_GPIO_PAD_BASE, GPIO_PAD_RDSEL_MASK, tmp);
			...
			reg |= tmp << 18;
			sdr_get_field(MSDC1_GPIO_CMD_BASE, GPIO_PUPD_MASK | GPIO_R1_MASK | GPIO_R0_MASK, tmp);
			reg |= tmp << 8;
			sdr_get_field(MSDC1_GPIO_CMD_BASE, GPIO_SR_MASK, tmp);
			reg |= tmp << 17;
			sdr_get_field(MSDC1_GPIO_CMD_BASE, GPIO_MSDC_DRVN, tmp);
			reg |= tmp << 0;
			break;
		case 2:
			sdr_get_field(MSDC2_GPIO_PAD_BASE, GPIO_PAD_RDSEL_MASK, tmp);
			...
			reg |= tmp << 18;
			sdr_get_field(MSDC2_GPIO_CMD_BASE, GPIO_PUPD_MASK | GPIO_R1_MASK | GPIO_R0_MASK, tmp);
			reg |= tmp << 8;
			sdr_get_field(MSDC2_GPIO_CMD_BASE, GPIO_SR_MASK, tmp);
			reg |= tmp << 17;
			sdr_get_field(MSDC2_GPIO_CMD_BASE, GPIO_MSDC_DRVN, tmp);
			reg |= tmp << 0;
			break;
		default:
			break;
...
{	
	...
	switch(id){
		case 0:
			...
			reg |= tmp << 17;
			sdr_get_field(MSDC0_GPIO_DAT_BASE, GPIO_MSDC_DRVN, tmp);
			reg |= tmp << 0;
			break;
		case 1:
			sdr_get_field(MSDC1_GPIO_PAD_BASE, GPIO_PAD_RDSEL_MASK, tmp);
			...
			reg |= tmp << 18;
			sdr_get_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK | GPIO_DAT1_R1_MASK | GPIO_DAT1_R0_MASK, tmp);
			reg |= tmp << 8;
			sdr_get_field(MSDC1_GPIO_DAT_BASE, GPIO_SR_MASK, tmp);
			reg |= tmp << 17;
			sdr_get_field(MSDC1_GPIO_DAT_BASE, GPIO_MSDC_DRVN, tmp);
			reg |= tmp << 0;
			break;
		case 2:
			sdr_get_field(MSDC2_GPIO_PAD_BASE, GPIO_PAD_RDSEL_MASK, tmp);
			...
			reg |= tmp << 18;
			sdr_get_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK | GPIO_DAT1_R1_MASK | GPIO_DAT1_R0_MASK, tmp);
			reg |= tmp << 8;
			sdr_get_field(MSDC2_GPIO_DAT_BASE, GPIO_SR_MASK, tmp);
			reg |= tmp << 17;
			sdr_get_field(MSDC2_GPIO_DAT_BASE, GPIO_MSDC_DRVN, tmp);
			reg |= tmp << 0;
			break;
		default:
			break;
...
static void msdc_dump_register(struct msdc_host *host)
{
    ...
    if (msdc_suspend_flag == 0) {
		...
		/* dump gpio register */
		INIT_MSG("Rg[C00] MSDC0_GPIO_CLK        = 0x%.8x", sdr_read32(0xF0005000 + 0xC00));
		INIT_MSG("Rg[C10] MSDC0_GPIO_CMD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC10));
		INIT_MSG("Rg[C20] MSDC0_GPIO_DAT        = 0x%.8x", sdr_read32(0xF0005000 + 0xC20));
		INIT_MSG("Rg[C30] MSDC0_GPIO_PAD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC30));
	
		INIT_MSG("Rg[C40] MSDC1_GPIO_CLK        = 0x%.8x", sdr_read32(0xF0005000 + 0xC40));
		INIT_MSG("Rg[C50] MSDC1_GPIO_CMD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC50));
		INIT_MSG("Rg[C60] MSDC1_GPIO_DAT        = 0x%.8x", sdr_read32(0xF0005000 + 0xC60));
		INIT_MSG("Rg[C70] MSDC1_GPIO_DAT_PUPD   = 0x%.8x", sdr_read32(0xF0005000 + 0xC70));
		INIT_MSG("Rg[C80] MSDC1_GPIO_PAD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC80));
	} else {
		INFO_MSG("Reg[00] MSDC_CFG       = 0x%.8x", sdr_read32(base + 0x00));
		INFO_MSG("Reg[04] MSDC_IOCON     = 0x%.8x", sdr_read32(base + 0x04));
		INFO_MSG("Reg[08] MSDC_PS        = 0x%.8x", sdr_read32(base + 0x08));
		INFO_MSG("Reg[0C] MSDC_INT       = 0x%.8x", sdr_read32(base + 0x0C));
		INFO_MSG("Reg[10] MSDC_INTEN     = 0x%.8x", sdr_read32(base + 0x10));    
		INFO_MSG("Reg[14] MSDC_FIFOCS    = 0x%.8x", sdr_read32(base + 0x14));
		INFO_MSG("Reg[18] MSDC_TXDATA    = not read");                        
		INFO_MSG("Reg[1C] MSDC_RXDATA    = not read");
		INFO_MSG("Reg[30] SDC_CFG        = 0x%.8x", sdr_read32(base + 0x30));
		INFO_MSG("Reg[34] SDC_CMD        = 0x%.8x", sdr_read32(base + 0x34));
		INFO_MSG("Reg[38] SDC_ARG        = 0x%.8x", sdr_read32(base + 0x38));
		INFO_MSG("Reg[3C] SDC_STS        = 0x%.8x", sdr_read32(base + 0x3C));
		INFO_MSG("Reg[40] SDC_RESP0      = 0x%.8x", sdr_read32(base + 0x40));
		INFO_MSG("Reg[44] SDC_RESP1      = 0x%.8x", sdr_read32(base + 0x44));
		INFO_MSG("Reg[48] SDC_RESP2      = 0x%.8x", sdr_read32(base + 0x48));                                
		INFO_MSG("Reg[4C] SDC_RESP3      = 0x%.8x", sdr_read32(base + 0x4C));
		INFO_MSG("Reg[50] SDC_BLK_NUM    = 0x%.8x", sdr_read32(base + 0x50));
		INFO_MSG("Reg[58] SDC_CSTS       = 0x%.8x", sdr_read32(base + 0x58));
		INFO_MSG("Reg[5C] SDC_CSTS_EN    = 0x%.8x", sdr_read32(base + 0x5C));
		INFO_MSG("Reg[60] SDC_DATCRC_STS = 0x%.8x", sdr_read32(base + 0x60));
		INFO_MSG("Reg[70] EMMC_CFG0      = 0x%.8x", sdr_read32(base + 0x70));                        
		INFO_MSG("Reg[74] EMMC_CFG1      = 0x%.8x", sdr_read32(base + 0x74));
		INFO_MSG("Reg[78] EMMC_STS       = 0x%.8x", sdr_read32(base + 0x78));
		INFO_MSG("Reg[7C] EMMC_IOCON     = 0x%.8x", sdr_read32(base + 0x7C));            
		INFO_MSG("Reg[80] SD_ACMD_RESP   = 0x%.8x", sdr_read32(base + 0x80));
		INFO_MSG("Reg[84] SD_ACMD19_TRG  = 0x%.8x", sdr_read32(base + 0x84));      
		INFO_MSG("Reg[88] SD_ACMD19_STS  = 0x%.8x", sdr_read32(base + 0x88));
		INFO_MSG("Reg[90] DMA_SA         = 0x%.8x", sdr_read32(base + 0x90));
		INFO_MSG("Reg[94] DMA_CA         = 0x%.8x", sdr_read32(base + 0x94));
		INFO_MSG("Reg[98] DMA_CTRL       = 0x%.8x", sdr_read32(base + 0x98));
		INFO_MSG("Reg[9C] DMA_CFG        = 0x%.8x", sdr_read32(base + 0x9C));                        
		INFO_MSG("Reg[A0] SW_DBG_SEL     = 0x%.8x", sdr_read32(base + 0xA0));
		INFO_MSG("Reg[A4] SW_DBG_OUT     = 0x%.8x", sdr_read32(base + 0xA4));
		INFO_MSG("Reg[B0] PATCH_BIT0     = 0x%.8x", sdr_read32(base + 0xB0));            
		INFO_MSG("Reg[B4] PATCH_BIT1     = 0x%.8x", sdr_read32(base + 0xB4));
		INFO_MSG("Reg[C0] DAT0_TUNE_CRC  = 0x%.8x", sdr_read32(base + 0xC0));
		INFO_MSG("Reg[C4] DAT1_TUNE_CRC  = 0x%.8x", sdr_read32(base + 0xC4));
		INFO_MSG("Reg[C8] DAT2_TUNE_CRC  = 0x%.8x", sdr_read32(base + 0xC8));
		INFO_MSG("Reg[CC] DAT3_TUNE_CRC  = 0x%.8x", sdr_read32(base + 0xCC));
		INFO_MSG("Reg[D0] CMD_TUNE_CRC   = 0x%.8x", sdr_read32(base + 0xD0));
		INFO_MSG("Reg[D4] SDIO_TUNE_WIND = 0x%.8x", sdr_read32(base + 0xD4));
		INFO_MSG("Reg[E0] SD_PAD_CTL0    = 0x%.8x", msdc_dump_padctl0(host->id));        
		INFO_MSG("Reg[E4] SD_PAD_CTL1    = 0x%.8x", msdc_dump_padctl1(host->id));
		INFO_MSG("Reg[E8] SD_PAD_CTL2    = 0x%.8x", msdc_dump_padctl2(host->id));
		INFO_MSG("Reg[EC] PAD_TUNE       = 0x%.8x", sdr_read32(base + 0xEC));
		INFO_MSG("Reg[F0] DAT_RD_DLY0    = 0x%.8x", sdr_read32(base + 0xF0));                        
		INFO_MSG("Reg[F4] DAT_RD_DLY1    = 0x%.8x", sdr_read32(base + 0xF4));
		INFO_MSG("Reg[F8] HW_DBG_SEL     = 0x%.8x", sdr_read32(base + 0xF8));
		INFO_MSG("Rg[100] MAIN_VER       = 0x%.8x", sdr_read32(base + 0x100));     
		INFO_MSG("Rg[104] ECO_VER        = 0x%.8x", sdr_read32(base + 0x104));                   
   
		/* dump gpio register */
		INFO_MSG("Rg[C00] MSDC0_GPIO_CLK        = 0x%.8x", sdr_read32(0xF0005000 + 0xC00));
		INFO_MSG("Rg[C10] MSDC0_GPIO_CMD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC10));
		INFO_MSG("Rg[C20] MSDC0_GPIO_DAT        = 0x%.8x", sdr_read32(0xF0005000 + 0xC20));
		INFO_MSG("Rg[C30] MSDC0_GPIO_PAD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC30));
	
		INFO_MSG("Rg[C40] MSDC1_GPIO_CLK        = 0x%.8x", sdr_read32(0xF0005000 + 0xC40));
		INFO_MSG("Rg[C50] MSDC1_GPIO_CMD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC50));
		INFO_MSG("Rg[C60] MSDC1_GPIO_DAT        = 0x%.8x", sdr_read32(0xF0005000 + 0xC60));
		INFO_MSG("Rg[C70] MSDC1_GPIO_DAT_PUPD   = 0x%.8x", sdr_read32(0xF0005000 + 0xC70));
		INFO_MSG("Rg[C80] MSDC1_GPIO_PAD        = 0x%.8x", sdr_read32(0xF0005000 + 0xC80));
	}
} 
...
void msdc_set_rdtdsel(struct msdc_host *host,bool sd_18)
{
    ...
    /* rdsel and tdsel is suggest to set 0 under MT6592 from DE */
    switch(host->id){
        case 0:
        ...
        case 1:
            if(sd_18){
                ...
                sdr_set_field(MSDC1_GPIO_PAD_BASE, GPIO_PAD_TDSEL_MASK, 0xa);
            } else{
                sdr_set_field(MSDC1_GPIO_PAD_BASE, GPIO_PAD_RDSEL_MASK, 0xc);
                sdr_set_field(MSDC1_GPIO_PAD_BASE, GPIO_PAD_TDSEL_MASK, 0xa);
            }
            break;
        case 2:
            sdr_set_field(MSDC2_GPIO_PAD_BASE, GPIO_PAD_RDSEL_MASK, 0x0);
            sdr_set_field(MSDC2_GPIO_PAD_BASE, GPIO_PAD_TDSEL_MASK, 0xa);
            break;
        default:
            break;
    }
...
static void msdc_pin_pud(struct msdc_host *host, int mode)
{
    switch(host->id){
		...
		case 1:
            switch (mode) {
                case MSDC_PULL_NONE:
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_R1_MASK, 0);
                    //sdr_set_field(MSDC0_GPIO_CMD_BASE, GPIO_PUPD_MASK, 0);
                    
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 0);

                    //sdr_set_field(MSDC0_GPIO_DAT_BASE, GPIO_PUPD_MASK, 0);
                    break;
                case MSDC_PU_10K:
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_R0_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_PUPD_MASK, 0);
                    
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_PUPD_MASK, 0);

                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK, 0);

                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_PUPD_MASK, 0);
					
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_PUPD_MASK, 0);

                    /* clock pull down with 50k during card init */
                    sdr_set_field(MSDC1_GPIO_CLK_BASE, GPIO_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_CLK_BASE, GPIO_R1_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_CLK_BASE, GPIO_PUPD_MASK, 1);
                    break;
                case MSDC_PU_50K:
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_R1_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_PUPD_MASK, 0);

                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_PUPD_MASK, 0);

                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK, 0);

                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_PUPD_MASK, 0);
					
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_PUPD_MASK, 0);
                ...
                case MSDC_PD_10K:
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_R0_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_PUPD_MASK, 1);
                    
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_PUPD_MASK, 1);

                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK, 1);

                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_PUPD_MASK, 1);
					
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 1);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_PUPD_MASK, 1);
                    break;
                case MSDC_PD_50K:
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_R0_MASK, 0);
                    sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_R1_MASK, 1);
		...
		case 2:
			switch (mode) {
                case MSDC_PULL_NONE:
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_R1_MASK, 0);
                    //sdr_set_field(MSDC0_GPIO_CMD_BASE, GPIO_PUPD_MASK, 0);
					
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 0);
			
                    //sdr_set_field(MSDC0_GPIO_DAT_BASE, GPIO_PUPD_MASK, 0);
                    break;
                case MSDC_PU_10K:
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_R0_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_PUPD_MASK, 0);
								
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_PUPD_MASK, 0);
			
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK, 0);
			
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_PUPD_MASK, 0);
								
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_PUPD_MASK, 0);
			
                    /* clock pull down with 50k during card init */
                    sdr_set_field(MSDC2_GPIO_CLK_BASE, GPIO_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_CLK_BASE, GPIO_R1_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_CLK_BASE, GPIO_PUPD_MASK, 1);
                    break;
                case MSDC_PU_50K:
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_R1_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_PUPD_MASK, 0);
			
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_PUPD_MASK, 0);
			
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK, 0);
			
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_PUPD_MASK, 0);
								
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_PUPD_MASK, 0);
                ...
                case MSDC_PD_10K:
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_R0_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_PUPD_MASK, 1);
								
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT0_PUPD_MASK, 1);
			
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK, 1);
			
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT2_PUPD_MASK, 1);
								
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 1);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_DAT_PUPD_BASE, GPIO_DAT3_PUPD_MASK, 1);
                    break;
                case MSDC_PD_50K:
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_R0_MASK, 0);
                    sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_R1_MASK, 1);
...
void msdc_set_driving(struct msdc_host *host,struct msdc_hw *hw,bool sd_18)
{
    switch(host->id){
        case 0:
			sdr_set_field(MSDC0_GPIO_CLK_BASE, GPIO_MSDC_DRVN, hw->clk_drv);
			sdr_set_field(MSDC0_GPIO_CMD_BASE, GPIO_MSDC_DRVN, hw->cmd_drv);
			sdr_set_field(MSDC0_GPIO_DAT_BASE, GPIO_MSDC_DRVN, hw->dat_drv);
			break;
        case 1:
            if(sd_18){
                sdr_set_field(MSDC1_GPIO_CLK_BASE, GPIO_MSDC_DRVN, hw->clk_drv_sd_18);
                sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_MSDC_DRVN, hw->cmd_drv_sd_18);
                sdr_set_field(MSDC1_GPIO_DAT_BASE, GPIO_MSDC_DRVN, hw->dat_drv_sd_18);
			}
			else{
                sdr_set_field(MSDC1_GPIO_CLK_BASE, GPIO_MSDC_DRVN, hw->clk_drv);
                sdr_set_field(MSDC1_GPIO_CMD_BASE, GPIO_MSDC_DRVN, hw->cmd_drv);
                sdr_set_field(MSDC1_GPIO_DAT_BASE, GPIO_MSDC_DRVN, hw->dat_drv);
            }
            break;
        case 2:
            sdr_set_field(MSDC2_GPIO_CLK_BASE, GPIO_MSDC_DRVN, hw->clk_drv);
            sdr_set_field(MSDC2_GPIO_CMD_BASE, GPIO_MSDC_DRVN, hw->cmd_drv);
            sdr_set_field(MSDC2_GPIO_DAT_BASE, GPIO_MSDC_DRVN, hw->dat_drv);
            break;
        default:
            break;
		}
...
#endif /* end of MTK_MT8193_SUPPORT */
...
static void msdc_set_timeout(struct msdc_host *host, u32 ns, u32 clks)
{
    ...
    N_MSG(OPS, "Set read data timeout: %dns %dclks -> %d x 1048576  cycles", ns, clks, timeout + 1);
}
...
static void msdc_tasklet_card(unsigned long arg)
{
	...
	if(hw->flags & MSDC_UHS1){
		host->mmc->caps |= MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 |	MMC_CAP_UHS_SDR50;   // | MMC_CAP_SET_XPC_180;   | MMC_CAP_UHS_SDR104
		if ( (hw->flags & MSDC_SD_NOSUPPORT_104) != MSDC_SD_NOSUPPORT_104) {
...
#ifdef MTK_SDIO30_ONLINE_TUNING_SUPPORT
static void ot_workqueue_func(struct work_struct *data)
{
...
out:
    queue_delayed_work(host->ot_wq, (struct delayed_work *)&host->ot_work, msecs_to_jiffies(OT_PERIOD));
}
#endif // MTK_SDIO30_ONLINE_TUNING_SUPPORT
...
static void msdc_pin_config(struct msdc_host *host, int mode)
{
    ...
    u32 base = host->base;
#endif
    int pull = (mode == MSDC_PIN_PULL_UP_10K || mode == MSDC_PIN_PULL_UP_50K) ? GPIO_PULL_UP : GPIO_PULL_DOWN;
    ...
    case MSDC_PIN_PULL_UP_10K:
		...
		#else
		msdc_pin_pud(host,MSDC_PU_10K);
		#endif
        break;
    case MSDC_PIN_PULL_UP_50K:
#ifdef FPGA_PLATFORM
            sdr_set_field(MSDC_PAD_CTL1, MSDC_PAD_CTL1_CMDPU, 1);
            sdr_set_field(MSDC_PAD_CTL1, MSDC_PAD_CTL1_CMDPD, 0);
            sdr_set_field(MSDC_PAD_CTL2, MSDC_PAD_CTL2_DATPU, 1);
        ...
        default:
            ...
            sdr_set_field(MSDC_PAD_CTL2, MSDC_PAD_CTL2_DATPD, 0);
#else
		msdc_pin_pud(host, MSDC_PULL_NONE);
#endif
            break;
    }
...
//#endif
            ...
            if((g_emmc_mode_switch == 0) && host->hw->host_function == MSDC_EMMC && host->mmc->card && mmc_card_mmc(host->mmc->card)){
                msdc_reset_hw(host->id);
                msdc_pin_reset (host, MSDC_PIN_PULL_UP);
                msdc_pin_config(host, MSDC_PIN_PULL_UP_10K);
                host->power_control(host,1);
                mdelay(10);
                                      ...
                                      unsigned long       timeout)
{
    ...
    N_MSG(CMD, "CMD<%d><0x%.8x> Arg<0x%.8x> %d blks", opcode , rawcmd, cmd->arg, (cmd->data==NULL)? 0 :cmd->data->blocks);
...
static int ot_process(struct msdc_host *host, struct ot_data *potData, u32 rawcmd, u32 rawarg, u32 CRCPass, bool rw)
{
    ...
    {
        xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "CRC Pass, intsts = 0x%x\n", intsts);
        return 0;
    }
    ...
    {
        //xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "dat0 delay = %d, host->cmd->resp[0] = 0x%x, CRC Pass = 0x%x\n", dat0rddly, host->cmd->resp[0], CRCPass);
    ...
    {
        //xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "dat1 delay = %d, host->cmd->resp[1] = 0x%x, CRC Pass = 0x%x\n", dat1rddly, host->cmd->resp[1], CRCPass);
    ...
    {
        //xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "dat2 delay = %d, host->cmd->resp[2] = 0x%x, CRC Pass = 0x%x\n", dat2rddly, host->cmd->resp[2], CRCPass);
    ...
    {
        //xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "dat3 delay = %d, host->cmd->resp[3] = 0x%x, CRC Pass = 0x%x\n", dat3rddly, host->cmd->resp[3], CRCPass);
...
_retry:
    ...
    else if(otData.dat0rddlypass == 0)
    {
...
#define SND_CMD 1
    ...
    if(host->hw->host_function == MSDC_SDIO) {
        spin_lock_irqsave(&host->clk_gate_lock, flags);
        if(host->ot_work_in_wq == 0) {
            queue_delayed_work(host->ot_wq, (struct delayed_work *)&host->ot_work, msecs_to_jiffies(OT_PERIOD));
            host->ot_work_in_wq = 1;
        }
        spin_unlock_irqrestore(&host->clk_gate_lock, flags); 
    }
#endif    // MTK_SDIO30_ONLINE_TUNING_SUPPORT
...
static int msdc_ops_switch_volt(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    if(ios->signal_voltage != MMC_SIGNAL_VOLTAGE_330){
        ...
        if ((sdr_read32(MSDC_PS) & ((1 << 24) | (0xF << 16))) == 0) {
            ...
            msdc_set_mclk(host, 0, 400000); /*For FPGA 13MHz clock,this not work*/
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if(hw->flags & MSDC_UHS1){
        mmc->caps |= MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 |MMC_CAP_UHS_SDR50;    // | MMC_CAP_SET_XPC_180;
		if ( (hw->flags & MSDC_SD_NOSUPPORT_104) != MSDC_SD_NOSUPPORT_104) {
			mmc->caps |= MMC_CAP_UHS_SDR104;
		}
    ...
    host->ot_work_in_wq = 0;
    host->ot_wq = create_workqueue("ot_queue");
    host->ot_work.host = host;
    INIT_DELAYED_WORK((struct delayed_work *)(&host->ot_work), ot_workqueue_func);
#endif // MTK_SDIO30_ONLINE_TUNING_SUPPORT
...
#ifdef MTK_SDIO30_ONLINE_TUNING_SUPPORT
    flush_workqueue(host->ot_wq);
    destroy_workqueue(host->ot_wq);
#endif // MTK_SDIO30_ONLINE_TUNING_SUPPORT
    if (mem)
